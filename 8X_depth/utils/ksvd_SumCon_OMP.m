...
% Given two training data matrices X: N x T and Y: N x T (same number of
% columns), we want to find a decomposition
%
%      X = Psi_cx*Z_c + Psi_x*Z_x
%      Y = Psi_cy*Z_c + Psi_y*Z_y
%
% where all the dictionaries Psi_cx, Psi, Psi_cy, and Psi_y have dimensions 
% N x K. The matrices Z_c, Z_x, and Z_y are sparse and contain the coefficients
% that represent X and Y in the corresponding dictionaries. 
% 
% Method:
% This is a coupled dictionary learning problem. We propose an algorithm adapted from K-SVD
% to train these dictionaries via solving.
%
%   minimize 0.5*||X - Psi_cx*Z_c - Psi_x*Z_x||_F^2 + ||Y - Psi_cy*Z_c - Psi_y*Z_y||_F^2
%	subject to ||Z_c||_0 + ||Z_x||_0 + ||Z_y||_0 <= s
% 
% We alternatively train coupled dicts and unique dicts with updating all
% sparse representations after each iteration.
% In the first phase, we fix the unique dicts, i.e. Psi and Phi, 
% and then only train the coupled dicts, i.e., Psi_c and Phi_c 
% and update all of sparse representations, i.e., Z U, V. 
% Then in the next phase, we fix the common dicts, i.e., Psi_c and Phi_c, 
% and then only train the unique dicts, i.e. Psi and Phi 
% and update all of sparse representations, i.e., Z U, V. 
% Then return to the first stage.
...
for main_iter = 1:Main_Iternum
	...
	for iter = 1:Sub_Iternum
		...
		Gamma = sparsecode(data,D,XtX,G,thresh); %768x11367
		Gam = full(Gamma);
